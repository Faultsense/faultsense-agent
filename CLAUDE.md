...
- **Troubleshooting — "every test in a harness timed out in pending".** This fingerprint means the harness served an empty or missing `/faultsense-agent.min.js`. Two common causes: (1) `dist/` hasn't been built — run `npm run build`; (2) a Docker harness (hotwire, livewire, liveview) is reusing a stale container whose bind mounts reference a previous repo location (e.g., pre-monorepo layout), and docker-compose didn't detect the drift because the compose file text is unchanged — run `npm run conformance:reset` to tear down and recreate all Docker harnesses. The `standardBeforeEach` helper in `conformance/shared/runners.ts` now guards against this by failing fast with an actionable message instead of letting every test time out silently.
