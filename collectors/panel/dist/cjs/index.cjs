/*! Faultsense panel collector (cjs/index) v0.5.4 | FSL-1.1-ALv2 | https://faultsense.com */
"use strict";
...
var xrayTabContent = null;
var streamTabContent = null;
var state = "none";
var activeTab = "stream";
var xrayActive = false;
var hoveredElement = null;
var buffer = [];
var badgePassCount = 0;
var badgeFailCount = 0;
var xrayHostElement = null;
var xrayShadowRoot = null;
var xrayOverlay = null;
var xrayObserver = null;
var dotMap = /* @__PURE__ */ new Map();
var rafId = 0;
var PANEL_CSS = `
  :host {
    all: initial;
...
function createPanel() {
  ...
  const existing = document.getElementById("fs-panel-host");
  if (existing) {
...
function renderDetailCard(element) {
  if (!xrayTabContent) return;
  xrayTabContent.innerHTML = "";
...
function renderRow(payload) {
  ...
  detailText += `  \xB7  ${payload.assertion_trigger}`;
  const modKeys = Object.keys(payload.assertion_type_modifiers || {});
  if (modKeys.length > 0) {
    const modStr = modKeys.filter((k) => k !== "timeout" && k !== "mpa" && k !== "mutex").map((k) => `${k}=${payload.assertion_type_modifiers[k]}`).join(", ");
    if (modStr) {
      detailText += `  \xB7  [${modStr}]`;
    }
  }
  ...
  if (payload.user_context) {
    ...
    userCtx.textContent = Object.entries(payload.user_context).map(([k, v]) => `${k}=${v}`).join(", ");
